#!/usr/bin/env python3
"""
MO1 Stage 2 optimization — off-TF expansion (PARALLEL).

Runs full param-grid re-optimization on the 3 off-timeframes (15m, 1h, 12h)
for all MO1 Stage 1 combos with OOS Sharpe >= 0.5.

Usage:
    python run_mo1_stage2_parallel_v2.py
    python run_mo1_stage2_parallel_v2.py --workers 8
    python run_mo1_stage2_parallel_v2.py --skip-download
"""
import sys
import os
import argparse
...
def main():
    parser = argparse.ArgumentParser(
        description="MO1 Stage 2 — off-TF expansion (15m, 1h, 12h)")
