#!/usr/bin/env python3
"""
MO1 Stage 1 optimization — vectorbt v2 (PARALLEL).
4H momentum rotation. Cross-asset RSI ranking approximated as per-asset
RSI level vs 50±momentum_threshold (removes multi-symbol dependency while
preserving the same parameter surface). 216 outer combos.
"""
import sys
import os
...
SL_PARAM_GRID = {
    "embedded":     {"atr_stop_mult": [1.5, 2.0, 2.5, 3.0]},
...
def _make_signals(close, rsi, adx, momentum_threshold, adx_trend_confirm,
    ...
    cooldown_bars is noted but not enforced in vectorbt (approx).
    """
    n = len(close)
    ...
    # Entries on first bar condition becomes true (crossover approximation)
...
def _run_vbt_portfolio(close_series, le, lx, se, sx, sl_type, sl_params_list, atr):
    ...
    if sl_type == "embedded":
        # ATR trailing stop (fixed ATR hard stop approximation)
...
def _optimize_vbt(data, direction, sl_type):
    ...
    for (rsi_period, mom_thresh, adx_confirm, adx_fade, cooldown) in product(
        ...
        le, lx, se, sx = _make_signals(
            c, rsi, adx, mom_thresh, adx_confirm, adx_fade, direction
        )
        if le.sum() + se.sum() == 0:
            continue
...
def main():
    parser = argparse.ArgumentParser(description="MO1 Stage 1 v2 parallel optimization")
    ...
    print(f"MO1 Stage 1 v2 — Parallel ({args.workers} workers)")
