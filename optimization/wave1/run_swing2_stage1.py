...
# SWING2 sl_mode semantics:
#   embedded      — strategy's own ATR stop + ATR-based TP (stop_dist = atr * atr_stop_mult).
#                   Optimises atr_stop_mult and rr_ratio.
#   fixed_pct     — BaseStrategy overrides stop/TP with fixed-% params; signal exit suppressed.
#                   Optimises stop_loss_pct and take_profit_pct.
#   fixed_signal  — Same fixed-% stop/TP.  SWING2 has no extra indicator exit, so identical
#                   to fixed_pct in practice (included for cross-strategy consistency).
#   atr           — BaseStrategy computes stop = entry ± atr * atr_stop_mult; TP = stop_dist * rr_ratio.
#                   Identical formula to embedded for SWING2, but controlled by BaseStrategy.
GRIDS = {
    "embedded": {
        "bb_length":     [15, 20, 25],
        "bb_mult":       [1.8, 2.0, 2.2],
