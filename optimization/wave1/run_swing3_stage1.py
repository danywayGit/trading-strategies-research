#!/usr/bin/env python3
"""
SWING3 Stage 1 optimization across all available symbols.
...
# SWING3 sl_mode semantics:
#   embedded      — Supertrend trailing exit + ATR initial backstop (stop_dist = atr * atr_stop_mult).
#                   No fixed TP at entry.  Optimises atr_stop_mult, st_factor, etc.
#   fixed_pct     — Fixed-% SL/TP overrides strategy stop; Supertrend flip exit suppressed.
#                   Optimises stop_loss_pct and take_profit_pct.
#   fixed_signal  — Fixed-% SL/TP AND Supertrend flip exit both active (whichever fires first).
#                   Optimises stop_loss_pct and take_profit_pct.
#   atr           — ATR stop + R:R TP (BaseStrategy); Supertrend flip exit suppressed.
