#!/usr/bin/env python3
"""
RR1 Stage 1 optimization across all available symbols.
Home TF: 4H. Runs 3 directions × 1 SL type (embedded only) = 3 combos per symbol.
Note: RR1 uses self.buy(sl=...) directly, so only embedded sl_mode applies.
"""
import json
import sys
import numpy as np
...
# RR1 uses self.buy(sl=...) directly — only embedded mode applies
# sl_buffer_atr = ATR buffer added to BB level to place the stop
GRIDS = {
    "embedded": {
        "sl_mode": ["embedded"],
        ...
        "stoch_k": [14, 18, 21],
        "stoch_d": [3, 5],
        "sl_buffer_atr": [0.2, 0.3, 0.5, 0.8, 1.0, 1.5],
    },
}

STRATEGY_CLASS = STRATEGY_REGISTRY["rr1_range_mean_reversion"]
