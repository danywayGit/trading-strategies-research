#!/usr/bin/env python3
"""
SFP1 Stage 1 optimization across all available symbols.
Home TF: 5m. Runs 3 directions × 1 SL type (embedded only) = 3 combos per symbol.

SFP1 uses self.buy(sl=...) / self.sell(sl=...) directly and cannot support
fixed_pct, fixed_signal, or atr SL modes from BaseStrategy.
"""
import json
import sys
import numpy as np
...
# SFP1 is embedded-only: uses self.buy(sl=...) inline and cannot delegate
# stop management to BaseStrategy. Only embedded grid is run.
SL_TYPES = ["embedded"]
...
STRATEGY_CLASS = STRATEGY_REGISTRY["sfp1_swing_failure_pattern"]
