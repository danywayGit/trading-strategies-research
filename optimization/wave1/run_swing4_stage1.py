#!/usr/bin/env python3
"""
SWING4 Stage 1 optimization across all available symbols.
Home TF: 4H. Runs 3 directions × 3 SL types = 9 combos per symbol.
"""
import json
import sys
import numpy as np
...
# SWING4 sl_mode semantics:
#   embedded      — ATR stop + R:R TP (stop_dist = atr * atr_stop_mult; tp = stop_dist * rr_ratio).
#                   Optimises atr_stop_mult and rr_ratio.
#   fixed_pct     — Fixed-% SL/TP overrides strategy stop; no extra exit in SWING4 next().
#                   Optimises stop_loss_pct and take_profit_pct.
#   fixed_signal  — Same as fixed_pct (SWING4 has no indicator-based trailing exit).
#   atr           — BaseStrategy ATR stop + R:R TP; identical formula to embedded for SWING4.
#                   Provided for consistency; optimises atr_stop_mult wide range.
GRIDS = {
    "embedded": {
        "macd_fast":           [10, 12],
        "macd_slow":           [24, 26],
...
STRATEGY_CLASS = STRATEGY_REGISTRY["swing4_macd_divergence"]
