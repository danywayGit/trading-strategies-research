#!/usr/bin/env python3
"""
MO1 Stage 3 optimization — DOW filter (PARALLEL).
...
Usage:
    python run_mo1_stage3_parallel_v2.py
    python run_mo1_stage3_parallel_v2.py --workers 8
    python run_mo1_stage3_parallel_v2.py --skip-download
"""
import sys
import os
import argparse
...
def _run_vbt_portfolio(close_series, le, lx, se, sx, sl_type, sl_params_list, atr, freq="1h"):
    ...
    if sl_type == "embedded":
        # ATR trailing stop (hard stop approximation) — no TP, uses signal exits
...
def main():
    parser = argparse.ArgumentParser(
        description="MO1 Stage 3 — DOW filter (15m, 1h, 12h off-TFs)")
